package first.cake.controller.file;

import first.cake.domain.dto.chat.FileUploadDto;
import first.cake.service.file.S3FileService;
...
public class FileController {
    ...
    public FileUploadDto uploadFile(@RequestParam("file") MultipartFile file, @RequestParam("roomId") String roomId) {
        ...
        FileUploadDto fileReq = service.uploadFile(file, UUID.randomUUID().toString(), roomId);
    ...
    public ResponseEntity<byte[]> download(@PathVariable String fileName, @RequestParam("fileDir") String fileDir) {
        ...
        try {
            ...
            return service.getObject(fileDir, fileName);
        } catch (IOException e) {
